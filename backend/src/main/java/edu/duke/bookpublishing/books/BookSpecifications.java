...
public final class BookSpecifications {
   ...
   */
  public static Predicate buildQueryPredicate(Path<Book> bookPath, CriteriaBuilder cb,
      String query) {
  ...
  private static Predicate termMatchesAnyField(Path<Book> bookPath, CriteriaBuilder cb,
      String term) {
    ...
    if (hasAuthorPunctuation) {
      ...
      authorPredicate = cb.like(cb.function("REPLACE", String.class,
          cb.function("REPLACE", String.class,
              cb.function("REPLACE", String.class, cb.lower(bookPath.get("author").get("name")),
                  cb.literal("."), cb.literal("")),
              cb.literal("'"), cb.literal("")),
          cb.literal("-"), cb.literal("")), "%" + lowerTerm + "%");
    }

    return cb.or(cb.like(cb.lower(bookPath.get("title")), "%" + lowerTerm + "%"), authorPredicate,
