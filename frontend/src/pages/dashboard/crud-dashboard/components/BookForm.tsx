...
export default function BookForm(props: BookFormProps) {
  ...
  const handleDateFieldChange = React.useCallback(
    (fieldName: keyof BookFormState['values']) => (value: Dayjs | null) => {
      if (value?.isValid()) {
        ...
        <Grid container spacing={2} sx={{ mb: 2, width: '100%' }}>
          ...
          <Grid size={{ xs: 12, sm: 6 }} sx={{ display: 'flex' }}>
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                value={formValues.publicationDate ? dayjs(formValues.publicationDate) : null}
                onChange={handleDateFieldChange('publicationDate')}
